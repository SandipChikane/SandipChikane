...
export function createSupabaseClient({ url, serviceRoleKey, fetchImpl = fetch }) {
  ...
  return {
    ...
    async getStudentAccount(email) {
      const query = new URLSearchParams({
        email: `eq.${email}`,
        select: 'email,password_hash,name,college,created_at,updated_at',
        limit: '1',
      });
      const rows = await request(`student_accounts?${query}`);
      return rows?.[0] || null;
    },

    async upsertStudentAccount(record) {
      return request('student_accounts?on_conflict=email', {
        method: 'POST',
        headers: {
          Prefer: 'resolution=merge-duplicates,return=representation',
        },
    ...
    async listStudentAccountEmails() {
      const rows = await request('student_accounts?select=email');
