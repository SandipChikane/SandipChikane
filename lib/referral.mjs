...
import { REFERRAL_COMMISSION_BPS, REFERRAL_COMMISSION_RATE_LABEL } from './referral-rate.mjs';
import { defaultProductReferralSettings, parseProductReferralSettings } from './referral-settings.mjs';
import { withdrawalEligibility } from './withdrawals.mjs';
...
export function safeInternalPath(next) {
  ...
  const allowed = new Set(['/', '/index.html', '/dashboard.html', '/course.html', '/tpo.html', '/referrals.html', '/referrals']);
...
export function createReferralServices({ store, env = {}, now = () => new Date() }) {
  ...
  async function awardForPaidOrder({
    ...
    await recordQualifyingPurchase(buyerEmail, asNonNegativeInt(order.amount));
  ...
  async function applyRefund({ providerOrderId, providerPaymentId, refundedPaise, full = false }) {
    ...
    const already = asNonNegativeInt(commission.reversedPaise);
    const portion = proportionalReversePaise(commission.commissionPaise, refundedPaise, commission.amountPaidPaise);
    const nextReversed = Math.min(commission.commissionPaise, already + portion);
    if (nextReversed >= commission.commissionPaise) {
      return reverseCommission(commission, { reason: 'refund' });
    }
    const updated = await store.updateCommission(commission.id, { reversedPaise: nextReversed });
    await store.insertLedger({
      userEmail: commission.referrerEmail,
      type: 'REFUND_REVERSAL',
      direction: 'debit',
    ...
    return { reversed: true, partial: true, amountPaise: portion, commission: updated };
  }
  ...
  async function dashboardFor(email, { origin = '' } = {}) {
    ...
    const eligibility = withdrawalEligibility({
      availablePaise: balances.available,
    ...
    return {
      ...
      balances: {
        ...balances,
        withdrawn: balances.paid,
        total: balances.pending + balances.available + balances.reserved + balances.paid,
      },
      eligibility,
      commissionPercentBps: REFERRAL_COMMISSION_BPS,
      commissionRateLabel: REFERRAL_COMMISSION_RATE_LABEL,
      activity: activity.map((row) => ({
        kind: row.kind,
        summary: row.summary,
        createdAt: row.createdAt,
      })),
      recent: successful.slice(0, 20).map((row) => ({
        id: row.id,
        status: row.status,
      ...
      withdrawals: withdrawals.slice(0, 20).map((row) => ({
        id: row.id,
      ...
      disclosure: `Earn ${REFERRAL_COMMISSION_RATE_LABEL} referral commission on every eligible successful direct referral purchase. Referral rewards are subject to eligibility, payment verification, refund checks, fraud checks and referral program terms. Earnings are not guaranteed.`,
  ...
  async function recordQualifyingPurchase(email, amountPaise) {
    const amount = asNonNegativeInt(amountPaise);
    if (!email || amount <= 0) return null;
    const profile = await ensureProfile(email);
    if (asNonNegativeInt(profile.withdrawalThresholdPaise) > 0) return profile;
    if (store.setWithdrawalThreshold) {
      return store.setWithdrawalThreshold(email, amount);
    }
    return store.upsertProfile({
      ...profile,
      withdrawalThresholdPaise: amount,
      withdrawalThresholdSetAt: now().toISOString(),
    });
  }
...
function adminCommission(row) {
  return {
    ...
    reversedPaise: row.reversedPaise || 0,
