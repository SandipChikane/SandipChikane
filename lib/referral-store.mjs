...
import { REFERRAL_COMMISSION_BPS, REFERRAL_COMMISSION_TYPE } from './referral-rate.mjs';
...
export function createMemoryReferralStore(seed = {}) {
  ...
  return {
    ...
    async setWithdrawalThreshold(email, amountPaise) {
      const existing = state.profiles.get(email);
      if (!existing) return null;
      if (Number(existing.withdrawalThresholdPaise) > 0) return { ...existing };
      const saved = {
        ...existing,
        withdrawalThresholdPaise: amountPaise,
        withdrawalThresholdSetAt: new Date().toISOString(),
      };
      state.profiles.set(email, saved);
      return { ...saved };
    },
    async addAdminNotice(row) {
      const saved = {
        id: randomUUID(),
        kind: row.kind || 'WITHDRAWAL_REQUEST',
    ...
    async markNoticeRead(id) {
      const row = state.notices.find((item) => item.id === id);
      if (!row) return null;
      row.status = 'READ';
...
export function createSupabaseReferralStore({ request, env = {} }) {
  ...
  return {
    ...
    async setWithdrawalThreshold(email, amountPaise) {
      ...
      try {
        const rows = await request(
          `referral_profiles?email=eq.${encodeURIComponent(email)}&or=(withdrawal_threshold_paise.is.null,withdrawal_threshold_paise.eq.0)`,
          {
            method: 'PATCH',
            headers: { Prefer: 'return=representation' },
            body: {
              withdrawal_threshold_paise: amountPaise,
              withdrawal_threshold_set_at: new Date().toISOString(),
              updated_at: new Date().toISOString(),
            },
          },
        );
        const saved = Array.isArray(rows) ? rows[0] : rows;
        return saved ? fromProfile(saved) : existing;
      } catch (error) {
        if (isMissing(error)) return existing;
        throw error;
      }
    },
    async addAdminNotice(row) {
      try {
        const rows = await request('referral_admin_notices', {
          method: 'POST',
          headers: { Prefer: 'return=representation' },
          body: {
            kind: row.kind || 'WITHDRAWAL_REQUEST',
        ...
        return fromNotice(Array.isArray(rows) ? rows[0] : rows);
      } catch (error) {
    ...
    async listAdminNotices() {
      const rows = await safe(
        () => request('referral_admin_notices?select=*&order=created_at.desc&limit=80'),
        [],
      );
    ...
    async markNoticeRead(id) {
      try {
        const rows = await request(`referral_admin_notices?id=eq.${encodeURIComponent(id)}`, {
          method: 'PATCH',
          headers: { Prefer: 'return=representation' },
...
function toProductRow(courseId, row) {
  return {
    ...
    commission_source: 'GLOBAL_DEFAULT',
    commission_type: REFERRAL_COMMISSION_TYPE,
    fixed_commission_paise: 0,
    commission_percent_bps: REFERRAL_COMMISSION_BPS,
    max_commission_paise: 0,
...
function fromProfile(row) {
  return {
    ...
    withdrawalThresholdPaise: row.withdrawal_threshold_paise || 0,
    withdrawalThresholdSetAt: row.withdrawal_threshold_set_at || '',
    createdAt: row.created_at,
  };
}
...
function fromCommission(row) {
  ...
  return {
    ...
    reversedPaise: row.reversed_paise || 0,
    holdingDays: row.holding_days,
...
function toCommission(row) {
  return {
    ...
    reversed_paise: row.reversedPaise || 0,
...
function fromNotice(row) {
  if (!row) return null;
  return {
    id: row.id,
    kind: row.kind || 'WITHDRAWAL_REQUEST',
    withdrawalId: row.withdrawal_id || row.withdrawalId || '',
    userEmail: row.user_email || row.userEmail,
    amountPaise: row.amount_paise || row.amountPaise || 0,
    method: row.method || '',
