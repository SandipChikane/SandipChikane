  ...
  return {
    ...
    async publicCourse(query = {}, req) {
      ...
      try {
        ...
        if (query.section && unlocked) {
          const section = (course.sections || []).find((item) => item.slug === query.section || item.id === query.section);
          if (!section || (section.status !== 'published' && !canPreview)) {
