...
import { createCms, toAdminCourse, toPublicCourse } from './cms.mjs';
...
export function createAdminHandlers(deps = {}) {
  ...
  return {
    ...
    async login(req, body = {}) {
      const expected = adminPassword(envSource);
      if (!expected) {
        return { status: 503, body: { error: 'ADMIN_PASSWORD is not set.' } };
      }
      ...
      const passwordOk = passwordsMatch(body.password, expected);
    ...
    async grantEnrollment(req, body = {}) {
      ...
      await gate.supabase.upsertEnrollment({
        student_email: email,
        student_name: String(body.studentName || nameFromEmail(email)).trim(),
        college: String(body.college || '').trim(),
        course_id: courseId,
        course_name: courseName,
        paid: true,
      });
      await gate.cms.writeAudit('grant-enrollment', 'enrollment', email, { courseId });
      return { status: 200, body: { ok: true } };
    },
    ...
    async publicCourse(query = {}, req) {
      ...
      try {
        ...
        return {
          status: 200,
          body: {
            course: toPublicCourse(hydrated.row, hydrated.modules, hydrated.lessons),
