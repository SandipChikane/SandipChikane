...
export function createAdminHandlers(deps = {}) {
  ...
  return {
    ...
    async updateWithdrawal(req, body = {}) {
      ...
      const updated = await withdrawals.setWithdrawalStatus(body.id, body.status, body.adminNote || '');
      await gate.cms.writeAudit('withdrawal-status', 'withdrawal', body.id, { status: body.status });
